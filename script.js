...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (contestFilter) contestFilter.addEventListener('change', applyFilters);
});

function normalizeEntry(p) {
    const relPath = p.sol_path || p.solutionUrl || p.solutionUrl.replace('./', '');
    const solutionUrl = p.solutionUrl || (relPath ? `${REPO_URL}/blob/main/${encodeURI(relPath)}` : '#');
    const questionUrl = p.questionUrl || p.q_link || '#';
    const id = p.problemId || p.id || 'N/A';
    const name = p.problemName || p.name || id;
    const rating = Number(p.rating) || 800; // Fallback low
    const tags = Array.isArray(p.tags) ? p.tags : ['implementation'];
    const contest = p.contest || 'unknown';
    return {
        problemId: String(id),
...
function renderStatistics(solutions) {
    try {
        ...
        // Chart: Contest distribution
        const contestCounts = {};
        solutions.forEach(s => { contestCounts[s.contest] = (contestCounts[s.contest] || 0) + 1; });
        const categories = Object.keys(contestCounts).sort();

        const canvas = document.getElementById('contestChart');
        if (canvas) {
            ...
            contestChart = new Chart(ctx, {
                ...
                data: {
                    labels: categories,
                    datasets: [{ label: 'Solved', data: categories.map(c => contestCounts[c]), backgroundColor: '#e74c3c' }]
                },
...
function applyFilters() {
    ...
    const contestFilter = document.getElementById('contestFilter');
    if (!searchInput || !contestFilter) return;
    const q = searchInput.value.trim().toLowerCase();
    const contestVal = contestFilter.value;
    currentFilteredSolutions = allSolutions.filter(s => {
        const matchesQuery = !q || (`${s.problemId} ${s.problemName}`).toLowerCase().includes(q);
        const matchesContest = contestVal === 'all' || s.contest.toLowerCase().includes(contestVal);
        return matchesQuery && matchesContest;
    });
    if (currentSort.key) sortTable(currentSort.key, false);
    renderTable(currentFilteredSolutions);
    renderStatistics(currentFilteredSolutions);  // Dynamic update
}
...
function sortTable(key, toggle = true) {
    try {
        ...
        const activeTh = document.querySelector(`[data-sort-key="${key}"] .fa-sort`);
        if (activeTh) {
